...
use App\Mpgroup;
use File;
class MpgroupController extends Controller
{
    ...
    public function getDanhsach(){
              $data=Mpgroup::select( 'id_admin_group', 'nhom_quantri', 'permissions')->orderBy('id_admin_group','DESC')->get()->toArray();
    ...
    public function getThem(){
    	...
    	return view('backend.nhomquantri.them',['data'=>$data]);
    }
    // thêm
  ...
  $parent=Mptheloai::where('parent_id',$id)->count();
    $tontai=Mptheloai::where('id_danhmuc',$id)->count();
     if($tontai==0){
      ...
      if($parent==0){
        ...
        $theloai->delete();
        return redirect()->route('backend.nhomquantri.danhsach')->with(['flash_level'=>'success','flash_message'=>'<h3>Xóa dữ liệu "(<b class="text-primary">'. $theloai->ten_danhmuc.'</b>)" thành công </h3>']);
        }
        else
        {
    ...
    $parent=Mptheloai::select('id_danhmuc','ten_danhmuc','parent_id')->get()->toArray();
      ...
      return view('backend.nhomquantri.sua',compact('parent','data'));
    }
